...
import { Sparkles, ShoppingBag, User, Home, Flower, BookOpen, Heart, Menu, X, ClipboardList, LogOut, Bell, Package, LayoutDashboard, Tag } from "lucide-react";
...
const Header = () => {
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-pink-100 shadow-xs font-sans">
          ...
          {currentUser?.role === "admin" ? (
            <div className="flex items-center gap-3 cursor-default">
              <div className="bg-pink-100 p-1 rounded-full flex items-center justify-center border border-pink-200">
                <img src={logo} alt="Bloom & Bouquet" className="h-8 w-8 sm:h-9 sm:w-9 object-cover rounded-full bg-white p-0.5" />
              </div>
              <div className="flex flex-col">
                <span className="font-cursive font-bold text-xl sm:text-2xl text-pink-700 leading-none">Bloom & Bouquet</span>
                <span className="text-[9px] sm:text-[10px] text-pink-400 font-semibold tracking-wider mt-0.5">Pusat Kontrol Admin</span>
              </div>
            </div>
          ) : (
            <Link to="/" className="flex items-center gap-3 hover:opacity-90 transition">
              ...
              <div className="flex flex-col">
                <span className="font-cursive font-bold text-xl sm:text-2xl text-pink-700 leading-none">Bloom & Bouquet</span>
                <span className="text-[9px] sm:text-[10px] text-pink-400 font-semibold tracking-wider mt-0.5">The Best-Looking Flowers in Town</span>
              </div>
            </Link>
          )}

          {/* NAVIGASI MENU DESKTOP (Tampil di layar besar) */}
          <nav className="hidden lg:flex items-center gap-2 bg-pink-50/50 p-1.5 rounded-full border border-pink-100">
            {currentUser?.role === "admin" ? (
              <>
                <Link
                  to="/admin"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-1.5 ${isActive("/admin") || isActive("/admin/") ? "bg-gradient-to-r from-pink-500 to-rose-400 text-white shadow-md shadow-pink-200/50 scale-105" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                ...
                <Link
                  to="/admin/produk"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-1.5 ${isActive("/admin/produk") ? "bg-gradient-to-r from-pink-500 to-rose-400 text-white shadow-md shadow-pink-200/50 scale-105" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                ...
                <Link
                  to="/admin/pesanan"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-1.5 ${isActive("/admin/pesanan") ? "bg-gradient-to-r from-pink-500 to-rose-400 text-white shadow-md shadow-pink-200/50 scale-105" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                ...
                <Link
                  to="/admin/voucher"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-1.5 ${isActive("/admin/voucher") ? "bg-gradient-to-r from-pink-500 to-rose-400 text-white shadow-md shadow-pink-200/50 scale-105" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                  <Tag size={15} /> Voucher
                </Link>
                <Link
                  to="/admin/rangkai"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-1.5 ${isActive("/admin/rangkai") ? "bg-gradient-to-r from-pink-500 to-rose-400 text-white shadow-md shadow-pink-200/50 scale-105" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
              ...
              <>
                <Link
                  to="/"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition flex items-center gap-1.5 ${isActive("/") ? "bg-pink-500 text-white shadow-xs" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                ...
                <Link
                  to="/bunga"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition flex items-center gap-1.5 ${isActive("/bunga") ? "bg-pink-500 text-white shadow-xs" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                ...
                <Link
                  to="/arti-bunga"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition flex items-center gap-1.5 ${isActive("/arti-bunga") ? "bg-pink-500 text-white shadow-xs" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
                ...
                <Link
                  to="/rangkai-buket"
                  className={`px-4 py-2 rounded-full text-xs font-bold transition flex items-center gap-1.5 ${isActive("/rangkai-buket") ? "bg-pink-500 text-white shadow-xs" : "text-pink-700 hover:bg-pink-100/60"
                    }`}
                >
        ...
        {/* DROPDOWN MENU MOBILE SAAT IKON HAMBURGER DIKLIK */}
        {mobileMenuOpen && (
          <div className="lg:hidden absolute top-20 left-0 w-full bg-white border-b border-pink-100 shadow-lg py-4 px-6 flex flex-col space-y-3 animate-fadeIn">
            {currentUser?.role === "admin" ? (
              <>
                <Link
                  to="/admin"
              ...
              <>
                <Link
                  to="/"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${isActive("/") ? "bg-pink-500 text-white" : "text-pink-700 hover:bg-pink-50"
                    }`}
                >
                ...
                <Link
                  to="/bunga"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${isActive("/bunga") ? "bg-pink-500 text-white" : "text-pink-700 hover:bg-pink-50"
                    }`}
                >
                ...
                <Link
                  to="/arti-bunga"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${isActive("/arti-bunga") ? "bg-pink-500 text-white" : "text-pink-700 hover:bg-pink-50"
                    }`}
                >
                ...
                <Link
                  to="/rangkai-buket"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${isActive("/rangkai-buket") ? "bg-pink-500 text-white" : "text-pink-700 hover:bg-pink-50"
                    }`}
                >
                ...
                <Link
                  to="/pesanan"
                  onClick={(e) => handleProtectedAction("/pesanan", e)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${isActive("/pesanan") ? "bg-pink-500 text-white" : "text-pink-700 hover:bg-pink-50"
                    }`}
                >
